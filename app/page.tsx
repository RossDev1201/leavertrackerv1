...
export default function HomePage() {
  ...
  // 🔐 derive role/info from session INSIDE the component
  const role = (session?.user as any)?.role ?? "unknown";
  const sessionEmployeeId = (session?.user as any)?.employeeId ?? null;
  ...
  // Load employees once the user is authenticated
  useEffect(() => {
  ...
  async function handleAddLeave(employeeId: string) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(json?.error || "Failed to submit leave.");
      }

      // Admin gets updated employees list; member only gets message
      if (json?.employees && isAdmin) {
      ...
      setForms((prev) => ({
        ...
        [employeeId]: {
          ...
          success:
            json?.message ||
            (isAdmin
              ? "Leave added and balance updated."
  ...
  // Show a loading screen while NextAuth checks the session
  if (status === "loading") {
    return (
  ...
  // While redirecting unauthenticated users, render nothing here
  if (status !== "authenticated") {
    return null;
  }
